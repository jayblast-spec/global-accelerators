...
import { ExternalLink, Bookmark, BookmarkCheck, Globe, DollarSign, Clock } from "lucide-react";
  ...
  return (
    ...
    >
      <Link href={`/accelerator/${a.slug}`} className="block group">
        <div
          className="rounded-2xl border p-5 transition-all duration-200 hover:border-accent/40 hover:shadow-lg cursor-pointer h-full flex flex-col"
        ...
        >
          ...
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <h3 className="text-sm font-semibold truncate" style={{ color: "var(--foreground)" }}>{a.name}</h3>
                {a.is_verified && (
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {a.region && (
                  <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: regionColor }}>
                    {a.region}
                  </span>
                )}
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                ...
                style={{
                  borderColor: saved ? "var(--accent)/40" : "var(--border)",
          ...
          {/* Quick stats */}
          <div className="flex items-center gap-3 mt-auto pt-3 border-t flex-wrap" style={{ borderColor: "var(--border)" }}>
            ...
            {a.equity_percentage !== null && (
              <span className="text-[11px]" style={{ color: "var(--muted)" }}>{a.equity_percentage}% equity</span>
            )}
            {a.program_duration_weeks !== null && (
              <div className="flex items-center gap-1">
                <Clock size={10} style={{ color: "var(--muted)" }} />
                <span className="text-[11px]" style={{ color: "var(--muted)" }}>{a.program_duration_weeks}w</span>
              </div>
            )}
            {a.batch_frequency && (
              <span className="text-[11px]" style={{ color: "var(--muted)" }}>{a.batch_frequency}</span>
            )}
            {a.remote_friendly && (
              <div className="flex items-center gap-1 ml-auto">
                <Globe size={10} style={{ color: "var(--accent-2)" }} />
                <span className="text-[11px]" style={{ color: "var(--accent-2)" }}>Remote OK</span>
              </div>
            )}
          </div>
        </div>
